...
module.exports.execute = function(processName, config) {
  ...
  function forkWorker() {
    ...
    workerPids.push(newWorker.process.pid);
  }
  ...
  function messageFromWorker(msg) {
    switch(msg) {
      case "noprivs":
        log('Go a noprivs message from a worker - shutting down');
        cleanUp();
        break;
      case "checkAlive":
        break;
      default:
        log("Unrecognised message from worker", msg);
        break;
    }
  }
  function cleanUp() {
    log('Master received SIGTERM.');
    // Stop listening, especially for Death events
    log('Removing all cluster listeners.');
    cluster.removeAllListeners();
    // Kill all workers. Node will do this itself if the master dies, but not if we call cleanUp in other ways
  ...
  if (cluster.isMaster) {
    ...
    // Downgrade priviledges for workers, if the config's not falsy (i.e. set as false, null, etc if you don't want to downgrade privs)
